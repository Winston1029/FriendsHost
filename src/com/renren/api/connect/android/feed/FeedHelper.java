...
//		}
	...
	public void asyncGet(Executor pool, 
			final FeedExtractRequestParam feed,
			final AbstractRequestListener<FeedExtractResponseBean> listener,
			boolean truncOption) {
		pool.execute(new Runnable() {
			@Override
			public void run() {
				try {
					FeedExtractResponseBean bean = get(feed);
					if (listener != null) {
						listener.onComplete(bean);
					}
				} catch (RenrenException rre) { // 参数、服务器等错误或异常
	...
	public FeedExtractResponseBean get(FeedExtractRequestParam feed) throws Throwable {
		if (!renren.isSessionKeyValid()) {
			String errorMsg = "Session key is not valid.";
			throw new RenrenException(RenrenError.ERROR_CODE_TOKEN_ERROR,
					errorMsg, errorMsg);
